...
export class CloudflarePuppeteerManager {
    ...
    async createSession(sessionId, options, retryCount = 0) {
        const MAX_RETRIES = 2;
        try {
            ...
            if (error instanceof Error && error.message.includes('Protocol error: Connection closed')) {
                if (retryCount < MAX_RETRIES) {
                    ...
                    throw new SessionError(`Browser connection failed after ${MAX_RETRIES + 1} attempts. This might indicate persistent network issues or Browser Rendering service problems. Try disconnecting and reconnecting to MCP Inspector. Error: ${error.message}`, sessionId);
                }
            }
            throw new SessionError(`Failed to create Cloudflare Puppeteer session ${sessionId}: ${error instanceof Error ? error.message : String(error)}`, sessionId);
        }
    }
    ...
    async navigateSession(sessionId, options) {
        ...
        try {
            ...
            throw new NavigationError(`Failed to navigate to ${options.url} with Cloudflare Puppeteer: ${error instanceof Error ? error.message : String(error)}`, options.url);
        }
    }
