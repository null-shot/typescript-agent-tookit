import { z } from 'zod';
import { SessionError, ExtractionError, NavigationError, } from "./schema.js";
import { browserErrorHandler } from "./browser-error-handler.js";
export function setupBrowserTools(server, browserManager, repository) {
    // Navigation tool
    server.tool("navigate", "Navigate to a URL in a browser session", {
        ...
        try {
            ...
            throw new NavigationError(`Navigation failed: ${error instanceof Error ? error.message : String(error)}`, args.url);
        }
    });
    // Screenshot tool
    server.tool("screenshot", "Take a screenshot of the current page or specific element", {
        ...
        return await browserErrorHandler.executeWithRetry(async () => {
            let page;
    ...
    // Extract text tool
    server.tool("extract_text", "Extract text content from the page using CSS selectors", {
        sessionId: z.string().optional().describe("Browser session ID"),
        ...
        try {
            ...
            if (args.selectors) {
                ...
                for (const [fieldName, selector] of Object.entries(args.selectors)) {
                    try {
                        if (args.multiple) {
                            extractedData[fieldName] = await page.$$eval(selector, (elements, attr) => elements.map(el => attr ? el.getAttribute(attr) : el.textContent?.trim()).filter(Boolean), args.attribute);
                        }
                        else {
                            extractedData[fieldName] = await page.$eval(selector, (element, attr) => attr ? element.getAttribute(attr) : element.textContent?.trim(), args.attribute);
                        }
                    }
                    catch (error) {
                ...
                try {
                    if (args.multiple) {
                        extractedData = await page.$$eval(args.selector, (elements, attr) => elements.map(el => attr ? el.getAttribute(attr) : el.textContent?.trim()).filter(Boolean), args.attribute);
                    }
                    else {
                        extractedData = await page.$eval(args.selector, (element, attr) => attr ? element.getAttribute(attr) : element.textContent?.trim(), args.attribute);
                    }
                }
                catch (error) {
            ...
            const createTextDisplay = (data, selectors) => {
             ...
             <div style="background: white; padding: 10px; border-radius: 4px; border: 1px solid #e0e0e0; max-height: 500px; overflow-y: auto;">
               ${Object.entries(data).map(([key, value]) => {
                     ...
                     <div style="background: white; padding: 8px; border-radius: 3px; border: 1px solid #e0e0e0;">
                       ${previewItems.map((item, index) => `<div style="padding: 4px 0; border-bottom: 1px solid #f0f0f0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif; line-height: 1.4;">
                     ...
                     <details style="margin-top: 8px;">
                       ...
                       <div style="margin-top: 6px; max-height: 200px; overflow-y: auto; background: white; padding: 6px; border-radius: 3px; border: 1px solid #e0e0e0;">
                         ${value.map((item, index) => `<div style="padding: 2px 0; font-size: 11px; font-family: monospace;">
             ...
             <div style="margin-top: 8px; font-size: 12px; color: #666;">
               ...
               <strong>Total items:</strong> ${Object.values(data).reduce((sum, val) => sum + (Array.isArray(val) ? val.length : 1), 0)}
             </div>
           </div>`;
                }
            ...
            throw new ExtractionError(`Text extraction failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    // Extract links tool
    server.tool("extract_links", "Extract all links from the page with optional filtering", {
        sessionId: z.string().optional().describe("Browser session ID"),
        url: z.string().optional().describe("URL to navigate to before extraction (optional if sessionId provided)"),
        ...
        try {
            ...
            const title = await page.title();
            // Extract all links
            const links = await page.evaluate((filter, internal, external, currentDomain) => {
            ...
            const createLinksDisplay = (linksData, filterText, internal, external) => {
                ...
                const filterInfo = [];
                if (filterText)
                    filterInfo.push(`Filter: "${filterText}"`);
                if (internal)
                    filterInfo.push('Internal only');
                if (external)
                    filterInfo.push('External only');
                return `<div style="border: 1px solid #ddd; border-radius: 8px; padding: 15px; margin: 10px 0; background: #f9f9f9;">
            ...
            <details style="margin-top: 15px;">
              ...
              <div style="margin-top: 8px; max-height: 300px; overflow-y: auto; background: white; padding: 8px; border-radius: 3px; border: 1px solid #e0e0e0;">
                ${linksData.map((link, index) => `<div style="padding: 4px 0; font-size: 10px; font-family: monospace; border-bottom: 1px solid #f0f0f0;">
            ...
            throw new ExtractionError(`Link extraction failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    });
    // Close session tool
    server.tool("close_session", "Close a browser session", {
        ...
        try {
            ...
            throw new SessionError(`Failed to close session: ${error instanceof Error ? error.message : String(error)}`, args.sessionId);
        }
    });
}
