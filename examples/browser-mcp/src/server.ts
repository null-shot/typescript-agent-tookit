...
export class BrowserMcpServer extends McpHonoServerDO<Env> {
  ...
  protected setupRoutes(app: any): void {
    ...
    app.get("/", (c: any) => {
      ...
      const mode = (this.env as any).BROWSER_MODE || "production";
      const browserType =
        mode === "cloudflare_puppeteer" && this.env.MYBROWSER
          ? "Cloudflare Puppeteer (unlimited testing)"
          : mode === "local_puppeteer"
            ? "Local Puppeteer (Node.js only)"
            : this.env.MYBROWSER
              ? "Cloudflare Browser Rendering (10min daily quota)"
      ...
      return c.json({
        ...
        documentation:
          "https://github.com/null-shot/typescript-agent-framework/tree/main/examples/browser-mcp",
      });
    });
    ...
    app.get("/.well-known/oauth-protected-resource", (c: any) => {
      return c.json({
        ...
        authorization_servers: [
          `${c.req.url}/.well-known/oauth-authorization-server`,
        ],
      });
    });
  ...
  async configureServer(server: McpServer): Promise<void> {
    ...
    try {
      if (mode === "cloudflare_puppeteer" && this.env.MYBROWSER) {
        ...
        console.log(
          "🔍 Using Cloudflare Puppeteer with Browser Rendering binding..."
        );
        try {
          browserManager = new CloudflarePuppeteerManager(this.env);
          browserType = "Cloudflare Puppeteer (unlimited testing)";
          console.log(
            "🚀 Using Cloudflare Puppeteer for unlimited browser automation"
          );
        } catch (error) {
          console.error("❌ Failed to initialize Cloudflare Puppeteer:", error);
          console.log(
            "⚠️ Falling back to mock browser manager due to Cloudflare Puppeteer initialization error"
          );
        ...
        console.log(
          "🔍 LocalPuppeteerManager.isAvailable() result:",
          isLocalAvailable
        );
        if (isLocalAvailable) {
          ...
          console.log(
            "🚀 Using Local Puppeteer for unlimited browser automation"
          );
        } else {
    ...
    console.log(
      `✅ Browser MCP server configured successfully with ${browserType}`
    );
  }
}
