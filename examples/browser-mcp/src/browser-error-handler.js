...
export class BrowserErrorHandler {
     ...
     */
    async executeWithRetry(operation, context, retryConfig = {}, cleanup) {
    ...
    classifyError(error, context) {
        ...
        if (message.includes('429') ||
            ...
            return new BrowserError(`Browser Rendering quota exceeded: ${error.message}`, 'QUOTA_EXCEEDED', context, false // Not retryable
            );
        }
        ...
        if (message.includes('connection closed') ||
            ...
            return new BrowserError(`Network/Connection error: ${error.message}`, 'NETWORK_ERROR', context, true // Retryable
            );
        }
        ...
        if (message.includes('failed to launch') ||
            ...
            return new BrowserError(`Browser launch failed: ${error.message}`, 'LAUNCH_FAILED', context, true // Retryable
            );
        }
        ...
        if (message.includes('navigation') ||
            ...
            return new BrowserError(`Page/Navigation error: ${error.message}`, 'PAGE_ERROR', context, true // Retryable
            );
        }
        ...
        if (message.includes('out of memory') ||
            ...
            return new BrowserError(`Resource error: ${error.message}`, 'RESOURCE_ERROR', context, context.attempt <= 2 // Only retry once for resource errors
            );
        }
        ...
        if (message.includes('session not found') ||
            ...
            return new BrowserError(`Session/Auth error: ${error.message}`, 'SESSION_ERROR', context, false);
        }
        // Unknown errors - conservatively retryable for first attempt only
        return new BrowserError(`Unknown browser error: ${error.message}`, 'UNKNOWN_ERROR', context, context.attempt === 1 // Only retry once for unknown errors
        );
    }
    /**
     * Creates a circuit breaker pattern for browser operations
     */
    createCircuitBreaker(failureThreshold = 5, resetTimeoutMs = 60000) {
        let failureCount = 0;
        ...
        return {
            async execute(operation, context) {
                ...
                if (state === 'open') {
                    throw new BrowserError('Circuit breaker is open - Browser Rendering service appears to be down', 'CIRCUIT_OPEN', { ...context, attempt: 1, totalAttempts: 1 }, false);
                }
                try {
                    const result = await operation();
